...
考虑一个简单的Interface，其可以与该类型的其他值进行比较：
...
与某些多态类型系统中的类似情况不同，类型`T`未实现` Equaler`。`t.equal`的参数类型是`T`，而不是实际上所需的类型`Equaler`。
...
在GO中，类型系统不会转换`Equal`的参数；这是程序员需要做的事情，如`T2`类型所示，它确实实现了`Equal`接口：
...
哪怕这样实现，也不像其他一些类型系统那样。因为在Go中，任何符合`Equaler`接口的对象都可以作为参数传递给`T2.Equal`。这样，在运行时，我们必须检查参数的类型是否是`T2`。一些语言将这些安排在编译时进行保证。
...
在Go中，`T3`并没有实现`Opener`接口，但在其他一些语言中是实现了的。
...
虽然在这种情况下，GO的类型系统对程序员的作用较少，但缺乏子类型使有关接口实现的规则非常清晰：函数的名称和签名是否完全是接口的名称和签名？ GO的规则也很容易高效地实施。 我们认为这些好处抵消了缺乏自动类型Promotion的情况。 如果有一天会采用某种形式的多态性键入，我们希望有一种表达这些示例的想法的方法，并在静态检查中检查它们。
...
不能直接转换。因为两种类型在内存中的存在形式不同。必须一个一个地拷贝到目标slice中。
...
在GO中，类型与方法紧密相关，因为每个命名类型都有一个（可能为空的）方法集。 一般规则是，您可以更改要转换的类型的名称（并因此更改其方法集），但您不能更改复合类型元素的名称（和方法集）。 GO要求您对类型转换有明确的说明。

### Why is my nil error value not equal to nil?

Under the covers, interfaces are implemented as two elements, a type `T` and a value `V`. `V` is a concrete value such as an `int`, `struct` or pointer, never an interface itself, and has type `T`. For instance, if we store the `int` value 3 in an interface, the resulting interface value has, schematically, (`T=int`, `V=3`). The value `V` is also known as the interface's *dynamic* value, since a given interface variable might hold different values `V` (and corresponding types `T`) during the execution of the program.

在底层的实现中，Interfaces的实现中存在两个元素，类型T和值V。V是一个具体的值，int、struct或者指针等，T表示Interfaces的类型。例如，如果我们存储一个int类型的3在接口中，这个接口的类型为int，值为3。

An interface value is `nil` only if the `V` and `T` are both unset, (`T=nil`, `V` is not set), In particular, a `nil`interface will always hold a `nil` type. If we store a `nil` pointer of type `*int` inside an interface value, the inner type will be `*int` regardless of the value of the pointer: (`T=*int`, `V=nil`). Such an interface value will therefore be non-`nil` *even when the pointer value `V` inside is* `nil`.

只有当V和T都没有设置时，接口才为nil。特别是，nil接口总是为nil类型。如果我们存储一个*int类型的nil指针在Interface中，此时接口的类型为`*int`，值为nil。这时接口并不等于nil。

This situation can be confusing, and arises when a `nil` value is stored inside an interface value such as an `error` return:

这种情况可能会令人困惑，并且当一个nil值存储在接口值中时会出现这种情况:
...
如果一切正常，该函数返回nil p，因此返回值是一个Error接口(`T=*MyError, V=nil`)。这意味着，如果调用者比较返回的错误，即使没有任何错误产生，也总是会看起来好像有错误。为了返回一个合适的nil error，函数必须返回一个显式的nil:
...
函数签名中返回error接口而不是一个具体的类型（例如*MyError）是个好主意。例如，os.Open
...
Most new code should live outside of the standard library and be accessible via the [`go` tool](https://go.dev/cmd/go/)'s `go get`command. Such code can have its own maintainers, release cycle, and compatibility guarantees. Users can find packages and read their documentation at [godoc.org](https://godoc.org/).
